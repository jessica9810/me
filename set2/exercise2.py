...
def set2exersise2(): # this is semi-right,the function definition is not properly formatted. 
    indices = [12 , 2, 26, 7, 0, 12, 12, 4, 17] # Corrected indices list
    wordArray = [getLetter(x) for x in indices]
    wordArray[0] = wordArray[0].upper()
    wordArray[1] = wordArray[1].upper() # <-- this should be using [] to index into a list
