    ...
    for word_length in range(3, 8):
        ...
        for _ in range(4):  # Get 4 words for each word length
            ...
            if r.status_code == 200:
                words.append(r.text.strip())  # Remove leading/trailing whitespaces from the fetched word
...
def random_filler_text(number_of_words=200) -> str:
    ...
    for _ in range(number_of_words):
        word_length = random.randint(3, 7)  # Random word length between 3 and 7 inclusive
        random_word = random.choice(my_dict[word_length])  # Randomly pick a word of that length
